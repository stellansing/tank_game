...
from modules.onlineConnetion import UDPNetwork, NetworkMessageHandler
import cfg
...
class MainGame:
    ...
    def start_multiplayer_game(self, mode='host', level='1'):
        ...
        # 初始化网络
        self.network = UDPNetwork(port=12345)

        if mode == 'host':
            if not self.network.create_host():
                print("创建主机失败")
                return
            print("已创建主机，等待玩家加入...")
        elif mode == 'join':
            if not self.network.join_game():
                print("加入游戏失败")
                return
            print("已连接到主机")

        # 初始化消息处理器
        self.message_handler = NetworkMessageHandler(self.network)
        self.message_handler.register_callbacks()
    ...
    def send_local_player_state(self):
        """发送本地玩家状态到网络"""
        if not self.message_handler or not self.my_tank:
            return

        # 序列化并发送坦克数据
        tank_data = self.message_handler.serialize_tank_data(self.my_tank)
        if tank_data:
            self.network.send_tank_update(tank_data)

        # 序列化并发送子弹数据
        bullets_data = self.message_handler.serialize_bullets_data(self.my_bullets)
        if bullets_data:
            self.network.send_bullet_update(bullets_data)

    def update_remote_players(self):
        """更新远程玩家的状态"""
        if not self.message_handler or not self.is_multiplayer:
            return

        # 这里可以处理远程玩家的数据，例如创建队友坦克
        remote_players = self.network.get_remote_players()
        for address, player_data in remote_players.items():
            if player_data['tank'] is not None:
                # 如果有远程坦克数据，可以在这里同步显示
                pass
